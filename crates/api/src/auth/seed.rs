//! 超级管理员 seed — 在数据库 schema 就绪后幂等地写入超级管理员账号。
//!
...
/// 超级管理员初始密码。
const SUPER_ADMIN_PASSWORD: &str = "$argon2id$v=19$m=19456,t=2,p=1$So5OK4RXsQNOzTdAVmZ/ig$wKE5S9ih1jFORT/EiylmgMTe8lus8Qxd+b6hQFa/ns0";
/// 超级管理员昵称。
const SUPER_ADMIN_NICKNAME: &str = "超级管理员";

/// 幂等地确保超级管理员账号存在。
///
/// 在 `ensure_schema` 之后调用。若账号已存在则跳过；否则用 argon2id 哈希
...
pub async fn ensure_super_admin(state: &SharedState) -> Result<(), AppError> {
    ...
    let user = domain::user::User {
        ...
        password_hash: SUPER_ADMIN_PASSWORD.to_owned(),
